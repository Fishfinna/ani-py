...
if os.name == 'nt':
    ...
    dll = ctypes.util.find_library(
        'mpv-2.dll') or ctypes.util.find_library('mpv-1.dll')
    if dll is None:
...
_handle_func('mpv_command_node_async',      [
             c_ulonglong, POINTER(MpvNode)],            c_int, ec_errcheck)
_handle_func('mpv_abort_async_command',     [
             c_ulonglong],                              None, errcheck=None)
...
_handle_func('mpv_event_to_node',           [POINTER(
    MpvNode), POINTER(MpvEvent)],      c_int, ec_errcheck, ctx=None)
...
def _make_node_str_map(d):
    ...
    char_ps = [(c_char_p(k.encode('utf-8')),
                c_char_p(_mpv_coax_proptype(v, str))) for k, v in d.items()]
    node_list = MpvNodeList(
        num=len(d),
...
class MPV(object):
        ...
        for event in _event_generator(self._event_handle):
            try:
                ...
                if eid == MpvEventID.COMMAND_REPLY:
                    ...
                    if callback:
                        with self._enqueue_exceptions():
                            callback(ErrorCode.exception_for_ec(
                    ...
                    for cb in list(self._command_reply_callbacks.values()):
                        with self._enqueue_exceptions():
                            cb(EventOverflowError(
                                'libmpv event queue has flown over because events have not been processed fast enough'), None)
    ...
    def wait_until_paused(self, timeout=None, catch_errors=True):
        ...
        self.wait_for_property(
            'core-idle', timeout=timeout, catch_errors=catch_errors)
    ...
    def wait_until_playing(self, timeout=None, catch_errors=True):
        ...
        waiting."""
        self.wait_for_property(
            'core-idle', lambda idle: not idle, timeout=timeout, catch_errors=catch_errors)
    ...
    def video_add(self, url, flags='select', title=None, lang=None, albumart=None):
        ...
        self.command('video_add', url.encode(fs_enc), *
                     _drop_nones(flags, title, lang, albumart))
        ...
        """ Register a custom stream protocol as documented in libmpv/stream_cb.h:
            ...
            class Stream:
                ...
                def seek(self, pos): # optional
                    # integer with new byte offset. The new offset may be before the requested offset
                    return new_offset
            ...
            def open_backend(_userdata, uri, cb_info):
                ...
                if hasattr(frontend, 'cancel'):
                    def cancel_backend(_userdata):
                        with self._enqueue_exceptions():
                            frontend.cancel()
                    cancel = cb_info.contents.cancel = StreamCancelFn(
                        cancel_backend)
